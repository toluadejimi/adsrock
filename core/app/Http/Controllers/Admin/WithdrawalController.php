...
class WithdrawalController extends Controller
{
    ...
    protected function withdrawalData($scope = null, $summary = false,$userId = null){
        ...
        if ($userId) {
            $withdrawals = $withdrawals->where('publisher_id',$userId);
        }

        $withdrawals = $withdrawals->searchable(['trx','publisher:username'])->dateFilter();
        ...
        if (!$summary) {
            return $withdrawals->with(['publisher','method'])->orderBy('id','desc')->paginate(getPaginate());
        }else{
            ...
            return [
                'data'=> $withdrawals->with(['publisher','method'])->orderBy('id','desc')->paginate(getPaginate()),
                'summary'=>[
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = Withdrawal::where('id',$request->id)->where('status',Status::PAYMENT_PENDING)->with('publisher')->firstOrFail();
