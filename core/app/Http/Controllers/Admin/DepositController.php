...
class DepositController extends Controller
{
    ...
    {
        if ($scope) {
            $deposits = Deposit::$scope()->with(['advertiser', 'gateway']);
        }else{
            $deposits = Deposit::with(['advertiser', 'gateway']);
        }

        if ($userId) {
            $deposits = $deposits->where('advertiser_id',$userId);
        }

        $deposits = $deposits->searchable(['trx','advertiser:username'])->dateFilter();
    ...
    {
        $deposit = Deposit::where('id', $id)->with(['advertiser', 'gateway'])->firstOrFail();
        $pageTitle = $deposit->advertiser->username.' requested ' . showAmount($deposit->amount);
    ...
    public function reject(Request $request)
    {
        ...
        notify($deposit->advertiser, 'DEPOSIT_REJECT', [
