                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToAdvertiser as $key => $toAdvertiser)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', @$sessionData['being_sent_to']) == $key)>{{ __($toAdvertiser) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none userCountText"> <i class="las la-info-circle"></i> <strong
                                            class="userCount">0</strong> @lang('active advertiser found to send the notification')</small>
                                </div>
                                    ...
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', @$sessionData['start']) }}"
                                                type="number" placeholder="@lang('Start form advertiser id. e.g. 1')" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', @$sessionData['batch']) }}"
                                                    type="number" placeholder="@lang('How many advertiser')" required>
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'TOPDEPOSITEDADVERTISERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited Publisher')</label>
                        <input class="form-control" type="number" name="number_of_top_deposited_advertiser" >
                    </div>
                    `);
                    return;
                }

                if (methodName == 'NOTLOGINUADVERTISERS') {
                    $('.input-append').html(`
                    <div class="form-group">
            ...
            function fetchAdvertiserList() {

                $('.row #user_list').select2({
                    ajax: {
                        url: "{{ route('admin.advertiser.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                        ...
                        processResults: function(response, params) {
                            ...
                            let data = response.advertisers.data;
                            return {
            ...
            function getAdvertiserCount(methodName) {
                ...
                var route = "{{ route('admin.advertiser.segment.count', ':methodName') }}"
