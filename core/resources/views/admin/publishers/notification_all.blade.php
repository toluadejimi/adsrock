            ...
            <div class="row  mb-4 justify-content-center">
                <div class="col-sm-7">
                    <div class="row gy-4 justify-content-center">
                        <div class="col-sm-6">
                            <x-widget link="javascript:void(0)" style="6" icon="fas fa-list" :title="$viaText . ' should be sent'" value="{{ @$sessionData['total_publisher'] }}"
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" name="being_sent_to" required data-minimum-results-for-search="1">
                                        @foreach ($notifyToPublisher as $key => $toPublisher)
                                            <option value="{{ $key }}" @selected(old('being_sent_to', @$sessionData['being_sent_to']) == $key)>{{ __($toPublisher) }}</option>
                                        @endforeach
                                    </select>
                                    <small class="text--info d-none userCountText"> <i class="las la-info-circle"></i> <strong
                                            class="userCount">0</strong> @lang('active publisher found to send the notification')</small>
                                </div>
                                    ...
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start" value="{{ old('start', @$sessionData['start']) }}"
                                                type="number" placeholder="@lang('Start form publisher id. e.g. 1')" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" value="{{ old('batch', @$sessionData['batch']) }}"
                                                    type="number" placeholder="@lang('How many publisher')" required>
    ...
    <script>
        ...
        (function($) {
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'TOPDEPOSITEDPUBLISHERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited Publisher')</label>
                        <input class="form-control" type="number" name="number_of_top_deposited_publisher" >
                    </div>
                    `);
                    return;
                }

                if (methodName == 'NOTLOGINUPUBLISHERS') {
                    $('.input-append').html(`
                    <div class="form-group">
            ...
            function fetchPublisherList() {
                ...
                $('.row #user_list').select2({
                    ajax: {
                        url: "{{ route('admin.publisher.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                        ...
                        processResults: function(response, params) {
                            ...
                            let data = response.publishers.data;
                            return {
            ...
            function getPublisherCount(methodName) {
                ...
                var route = "{{ route('admin.publisher.segment.count', ':methodName') }}"
