        ...
        <div class="col-md-12">
            <div class="card custom--card">
                <div class="card-header card-header-bg d-flex flex-wrap justify-content-between align-items-center">
                    <h5 class="text-dark mt-0 mb-0">
                        @php echo $myTicket->statusBadge; @endphp
                        [@lang('Ticket')#{{ $myTicket->ticket }}] {{ $myTicket->subject }}
                    </h5>

                    @if ($myTicket->status != Status::TICKET_CLOSE && $myTicket->publisher)
                        <button class="btn btn-dark close-button btn--sm confirmationBtn" type="button"
                            data-question="@lang('Are you sure to close this ticket?')" data-action="{{ route('ticket.close', $myTicket->id) }}"><i
                                class="fas fa-lg fa-times-circle"></i> @lang('Close')
                        </button>
                    @endif
                </div>
                <div class="card-body">
                        ...
                        <div class="row justify-content-between">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <textarea name="message" class="form--control" rows="4" placeholder="@lang('Message')" required>{{ old('message') }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-md-9">
                                ...
                                <p class="mb-2"><span class="text--info">
                                        @lang('Max 5 files can be uploaded |Maximum upload size is' . convertToReadableSize(ini_get('upload_max_filesize')) . ' | Allowed File Extensions: .jpg, .jpeg, .png, .pdf, .doc, .docx')</span></p>
...
@auth('publisher')
    @push('breadcrumb-plugins')
        <a href="{{ route('ticket.index') }}" class="btn btn--sm  btn-outline--base">
